...
int main(int argc, char * argv[])
{
    ...
    for(TString sf_it : signs_and_flavor_vec){
        cout << "----------" << sf_it << "-----------" << endl;
        cout << "\\scalebox{0.6}{" << endl;
        cout << "\\begin{tabular}{|c|c|c|c|}" << endl;
        cout << "\\hline" << endl;
        cout << "Process (" + sf_it + ") & $l_2$ Vtx & SVerr<0.2cm & Combined \\\\" << endl;
        cout << "\\hline" << endl;
        for(int i = 1; i < (argc + 1)/2; i++){
            TString name = (TString)argv[(argc - 1)/2 + i];
            TFile*  file = TFile::Open(argv[i]);
            TH1F* hist = (TH1F*) file->Get(sf_it+"_IVF_categories");
            cout << name << " & " << 100*accuracy*round(hist->GetBinContent(2)/hist->GetBinContent(1)/accuracy) << " & " << 100*accuracy*round(hist->GetBinContent(3)/hist->GetBinContent(2)/accuracy) << " & " << 100*accuracy*round(hist->GetBinContent(3)/hist->GetBinContent(1)/accuracy) << " \\\\" << endl;
        }
        cout << "\\hline" << endl;
