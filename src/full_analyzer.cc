    ...
    for(unsigned jentry = j_begin; jentry < j_end; ++jentry){
        ...
        if(sampleflavor == "e" or sampleflavor == "mu"){
            for(double V2 : reweighting_V2s){
                ...
                if(printevent) std::cout << "for " << V2 << ", we get: " << reweighting_weights[V2] << std::endl;
            }
        }
        ...
        for(auto& MassMap : JetTagVal){
            for(auto& V2Map : MassMap.second){
                ...
                if(_1e1disple_PFN[MassMap.first][V2Map.first]){
                    fill_histograms(&hists, &hists2D, signs_and_flavor + "_afterPFN" + MV2name[MassMap.first][V2Map.first], i_leading_e, i_subleading_displ_e);
                //    fill_KVF_histograms(&hists, &hists2D, signs_and_flavor + "_afterPFN", i_subleading_displ_e, i_gen_subleading_displ_e);
                    fill_IVF_histograms(&hists, &hists2D, signs_and_flavor + "_afterPFN" + MV2name[MassMap.first][V2Map.first], i_leading_e, i_subleading_displ_e, i_gen_subleading_displ_e);
                ...
                if(_1mu1displmu_PFN[MassMap.first][V2Map.first]){
                    fill_histograms(&hists, &hists2D, signs_and_flavor + "_afterPFN" + MV2name[MassMap.first][V2Map.first], i_leading_mu, i_subleading_displ_mu);
                //    fill_KVF_histograms(&hists, &hists2D, signs_and_flavor + "_afterPFN", i_subleading_displ_mu, i_gen_subleading_displ_mu);
                    fill_IVF_histograms(&hists, &hists2D, signs_and_flavor + "_afterPFN" + MV2name[MassMap.first][V2Map.first], i_leading_mu, i_subleading_displ_mu, i_gen_subleading_displ_mu);
                ...
                if(_1e1disple_TrainingPFN[MassMap.first][V2Map.first]){
                    fill_histograms(&hists, &hists2D, signs_and_flavor + "_TrainingHighPFN" + MV2name[MassMap.first][V2Map.first], i_leading_e, i_subleading_displ_e);
                //    fill_KVF_histograms(&hists, &hists2D, signs_and_flavor + "_TrainingHighPFN", i_subleading_displ_e, i_gen_subleading_displ_e);
                    fill_IVF_histograms(&hists, &hists2D, signs_and_flavor + "_TrainingHighPFN" + MV2name[MassMap.first][V2Map.first], i_leading_e, i_subleading_displ_e, i_gen_subleading_displ_e);
                }
                if(_1mu1displmu_TrainingPFN[MassMap.first][V2Map.first]){
                    fill_histograms(&hists, &hists2D, signs_and_flavor + "_TrainingHighPFN" + MV2name[MassMap.first][V2Map.first], i_leading_mu, i_subleading_displ_mu);
                //    fill_KVF_histograms(&hists, &hists2D, signs_and_flavor + "_TrainingHighPFN", i_subleading_displ_mu, i_gen_subleading_displ_mu);
                    fill_IVF_histograms(&hists, &hists2D, signs_and_flavor + "_TrainingHighPFN" + MV2name[MassMap.first][V2Map.first], i_leading_mu, i_subleading_displ_mu, i_gen_subleading_displ_mu);
                }
            }
        }
        //if(_1e1disple_BDT){
